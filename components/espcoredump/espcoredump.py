...
class ESPCoreDumpElfFile(esptool.ELFFile):
    ...
    def _read_sections(self, f, section_header_offs, shstrndx):
        ...
        if len(section_header) % LEN_SEC_HEADER != 0:
            print('WARNING: Unexpected ELF section header length {:04x} is not mod-{:02x}'.format(len(section_header), LEN_SEC_HEADER))
        ...
        if sec_type != esptool.ELFFile.SEC_TYPE_STRTAB:
            print('WARNING: ELF file has incorrect STRTAB section type 0x{:02x}'.format(sec_type))
        f.seek(sec_offs)
    ...
    def _read_program_segments(self, f, seg_table_offs, entsz, num):
        ...
        if len(seg_table) % LEN_SEG_HEADER != 0:
            print('WARNING: Unexpected ELF program header table length {:04x} is not mod-{:02x}'.format(len(seg_table),LEN_SEG_HEADER))
...
class ESPCoreDumpLoader(object):
    ...
    def _get_registers_from_stack(self, data, grows_down):
        ...
        # [MT]: I don't know why the 0x40000000 bit is forcefully set here. When crashing due to jumping to a non-code
        # address this causes the value to be incorrectly shown in gdb:
    ...
    def remove_tmp_file(self, fname):
        ...
        try:
            ...
            if e.errno != errno.ENOENT:
                print("Warning failed to remove temp file '{}' ({})!".format(fname, e.errno))
...
class GDBMIOutStreamHandler(GDBMIOutRecordHandler):
    ...
    def execute(self, ln):
        ...
        if self.func:
            ...
            self.func(ln[1:].strip(b'"').replace(b'\\n', b'\n').replace(b'\\t', b'\t'))
...
class GDBMIResultHandler(GDBMIOutRecordHandler):
    ...
    def _parse_rc(self, ln, rc):
        ...
        if ln.startswith(rc_str.encode('utf8')):
            self.result_class = rc
            sl = len(rc_str)
            if len(ln) > sl:
                self.result_str = ln[sl:]
                if self.result_str.startswith(b','):
...
def info_corefile(args):
    ...
    def gdbmi_console_stream_handler(ln):
        sys.stdout.write(ln.decode('utf8'))
        sys.stdout.flush()
    
    def gdbmi_read2prompt(f, out_handlers=None):
        while True:
            ln = f.readline().rstrip(b' \r\n')
            if ln == b'(gdb)':
                break
            elif len(ln) == 0:
                break
                ...
                for h in out_handlers:
                    if ln.startswith(out_handlers[h].TAG.encode('utf8')):
